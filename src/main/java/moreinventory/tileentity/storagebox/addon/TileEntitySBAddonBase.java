...
import moreinventory.network.packet.PacketSBAddonBase;
import moreinventory.network.packet.PacketSBAddonBaseConfig;
	   ...
	   public void readFromNBT(NBTTagCompound par1NBTTagCompound)
	    {
	        ...
	        if(par1NBTTagCompound.hasKey("isPrivate"))this.isPrivate = par1NBTTagCompound.getBoolean("isPrivate");
	        if(par1NBTTagCompound.hasKey("owner"))this.ownerName = par1NBTTagCompound.getString("owner");
	    }
        ...
        public void sendCommonGuiPacketToClient(byte channel,boolean flg){
            MoreInventoryMod.packetPipeline.sendPacketToServer(this.getCommonGuiPacket(channel , flg));
        }

	    public void sendCommonGuiPacketToServer(byte channel,boolean flg){
                MoreInventoryMod.packetPipeline.sendPacketToServer(this.getCommonGuiPacket(channel , flg));
	    }
		...
		protected AbstractPacket getCommonPacket(){
	    	return new PacketSBAddonBase(xCoord, yCoord, zCoord, isPrivate , ownerName);
		}
		
		protected AbstractPacket getCommonGuiPacket(byte channel,boolean flg){

            return new PacketSBAddonBaseConfig(xCoord, yCoord, zCoord, channel , flg);
		}
