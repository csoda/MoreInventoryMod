...
public class RendererTileEntityTransportManager extends TileEntitySpecialRenderer {
	...
	public void renderAModelAt(TileEntity tile, double d, double d1, double d2, float f) {
		...
		if(tile.getWorldObj() != null){
			...
			if(!flg&&(0<=time&&time<=4))light[0] = true;
			if(!flg&&(5<=time&&time<=9))light[1] = true;
			if(!flg&&(10<=time&&time<=14))light[2] = true;
			if(!flg&&(15<=time&&time<=19))light[3] = true;
			if(!flg)light[4] = true;
		}
	...
	{
		switch(rotation){
		case 1:  GL11.glRotatef(180F, 0.0F, 0.0F, 1.0F);break;
		case 2:  GL11.glRotatef(90F, -1.0F, 0.0F, 0.0F);break;
		case 3:  GL11.glRotatef(90F, 1.0F, 0.0F, 0.0F);break;
		case 4:  GL11.glRotatef(90F, 0.0F, 0.0F, -1.0F);break;
		case 5:  GL11.glRotatef(90F, 0.0F, 0.0F, 1.0F);break;
		}
	}
