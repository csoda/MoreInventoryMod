...
public class RendererTileEntityCatchall extends TileEntitySpecialRenderer {
	...
	public RendererTileEntityCatchall(){
	...
	public void renderAModelAt(TileEntityCatchall tile, double d, double d1, double d2, float f) {
		...
		byte rotation = 0;
		if(tile.getWorldObj() != null)rotation = (byte) tile.getBlockMetadata();
		...
		if(tile.getWorldObj() != null){
			...
			{
				GL11.glRotatef(90, -1.0F, 0.0F, 0.0F);
				GL11.glTranslatef((float)0.0F, (float)0.0F, 0.25F);
	...
	{
		this.renderAModelAt((TileEntityCatchall)par1TileEntity, par2, par4, par6, par8);
	}
}
