...
import moreinventory.item.inventory.InvChestTransporter;
...
public class ItemChestTransporter extends Item
{
	...
	public void addInformation(ItemStack itemstack, EntityPlayer player, List list, boolean bool)
	{
		...
		if(3<=dm&&dm<=18){
			InvChestTransporter invChestTP = new InvChestTransporter(itemstack);
    ...
    public boolean onItemUseFirst(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int par4, int par5, int par6, int par7, float par8, float par9, float par10)
    {
        ...
        if(par1ItemStack.getItemDamage()==0)
        {
        	...
        	if(MoreInventoryMod.transportableChest.containsKey(onBlock.getUnlocalizedName()))
        	{
        		...
        		if(tileEntity==null||!(tileEntity instanceof IInventory)||((IInventory)tileEntity).getSizeInventory() < 27)
        		{
        			return true;
        		}
        	}
        	...
        	InvChestTransporter invChestTP = new InvChestTransporter(par1ItemStack);
			if(invChestTP.transferToPlayer(tileEntity))
			{
				...
				{
					Map<Integer,Integer> IconMap = MoreInventoryMod.transportableChestIcon.get(onBlock.getUnlocalizedName());
